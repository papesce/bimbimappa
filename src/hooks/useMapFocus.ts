...
import { findOptimalRadius, getPlacesWithinRadius } from '../lib/geo';
...
function persist(
  ...
  try {
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ center, radius, cityName, stateName, countryCode }),
    );
  } catch {
    /* storage full — ignore */
  }
...
export function useMapFocus(places: Place[]) {
  ...
  useEffect(() => {
    ...
    reverseGeocode(center.lat, center.lng)
      ...
      .catch(() => {});
  }, [center, stateName, radius, countryCode, cityName]);
