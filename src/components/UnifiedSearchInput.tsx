import { Check, Clock, MapPin, Search, Trash2, X } from 'lucide-react';
...
export default function UnifiedSearchInput({
  ...
  const showDropdown = open && ((!hasCity && q.length === 0) || q.length >= 2);
    ...
    <div className={`unified-search${variant === 'topbar' ? ' unified-search--topbar' : ''}`}>
      ...
      {!hideRadius && (center || (viewportBounds && !center)) && (
        <div className="unified-search-radius-inline">
          ...
          <RadiusSelector
            value={center ? (radius ?? 0) : (boundsRadius ?? 0)}
            onChange={
              center ? (onRadiusChange ?? (() => {})) : (onBoundsRadiusChange ?? (() => {}))
            }
      ...
      {showDropdown && (
        <div
          ref={dropdownRef}
          className={`unified-search-dropdown${isScrollable ? ' unified-search-dropdown--scrollable' : ''}`}
        >
          {showRecent && (
            <>
          ...
          {showNoRecent && <div className="unified-search-empty">No recent searches</div>}
          {showCityResults && (
            <>
              <div className="unified-search-dropdown-header">
                <span>📍 Cities</span>
          ...
          {showNoResults && <div className="unified-search-empty">No results found</div>}
        </div>
      )}
    </div>
  );
