...
export default function MobileExplorer({
  ...
  return (
    <section
      className={`mobile-explorer${expanded ? ' expanded' : ' collapsed'}`}
    ...
    >
      ...
      <div className="mobile-explorer-heading">
        <div>
          ...
          <h2>
            {cityName ? `Near ${cityName}` : 'Places nearby'} <span>{visiblePlaces.length}</span>
          </h2>
        </div>
        <div className="mobile-explorer-heading-actions">
          <button
            type="button"
            className="mobile-explorer-filter"
          ...
          <button
            ...
            aria-label={expanded ? 'Collapse explorer' : 'Expand explorer'}
          >
            <ChevronDown className={expanded ? '' : 'closed'} size={17} />
          </button>
        </div>
      </div>
      ...
      {expanded && (
        <>
          ...
          <div className="mobile-explorer-actions">
            <button type="button" className="mobile-explorer-nearby" onClick={onNearMe}>
              <Navigation size={15} /> Near me
            </button>
            <span className="mobile-explorer-count">
          ...
          <div className="mobile-explorer-types" role="group" aria-label="Browse by type">
            <button
              type="button"
            ...
            {CATEGORIES.filter(item => item.key !== 'home').map(item => {
              ...
              return (
                <button
                  ...
                  onClick={() => onCategoryChange(category === item.key ? null : item.key)}
                >
                  {item.label}
                </button>
              );
          ...
          <div className="mobile-explorer-results">
            {visiblePlaces.map(place => {
              ...
              const distance = center
                ? getDistanceKm(center.lat, center.lng, place.lat, place.lng)
                : null;
              return (
                <article
                  className="mobile-explorer-card"
                  key={place.id}
                ...
                >
                  <div
                    className="mobile-explorer-card-icon"
                  ...
                  <div className="mobile-explorer-card-actions">
                    {primaryLink && (
                      <a
                        className="mobile-explorer-source"
                      ...
                      >
                        <BrandIcon url={primaryLink.url} size={17} />
                      </a>
                    )}
                    <a
                      className="mobile-explorer-navigate"
            ...
            {visiblePlaces.length === 0 && (
              <p className="mobile-explorer-empty">No places match. Try another type or search.</p>
            )}
          </div>
        </>
      )}
