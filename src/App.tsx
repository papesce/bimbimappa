import { Library, MapPin, Plus, Search, SlidersHorizontal, X } from 'lucide-react';
...
export default function App() {
  ...
  async function handleUpdate(
    ...
    if (removePhoto) {
      ...
      if (existing?.photo_url) {
        deletePlacePhoto(existing.photo_url).catch(err =>
  ...
  const handleLocatePlace = useCallback(
    ...
    [isMobile, setCenter, handleSelectPlace],
  );
  ...
  const setTripFocus = useCallback(
    (trip: Trip) => {
  ...
  const handleSearchSelectTrip = useCallback(
    (trip: Trip) => {
      setLibraryTab('trips');
  ...
  const handleMapSearchSelectTrip = useCallback(
    (trip: Trip) => {
      setPanel(null);
      setSheetState('hidden');
      setTripFocus(trip);
  ...
  const handleFocusBarRadiusChange = useCallback(
    (value: number) => {
      if (center) {
        setRadius(value);
        setViewportBounds(null);
  ...
  const handleCitySelect = useCallback(
    (lat: number, lng: number, name: string, state: string, cc = '') => {
      clearFocusEntity();
  ...
  const boundsCenter: GeoPoint | null =
  ...
  if (visitedFilter === 'visited') {
    filteredPlaces = filteredPlaces.filter(p => p.visited);
  ...
  const libraryPlaces = useMemo(() => {
    ...
    return filteredPlaces.filter(
      p =>
        ...
        (p.visited && 'visited'.includes(q)) ||
        (!p.visited && 'unvisited'.includes(q)),
    );
  ...
  const contextLabels = [
    ...
    visitedFilter === 'unvisited' ? 'Not visited' : '',
  ].filter(Boolean);
    ...
    : center
      ...
      : viewportBounds
        ? {
            ...
            onHover: boundsRadius
              ? hover => setHoverRadiusKm(hover ? boundsRadius : null)
              : undefined,
          }
        : null;
    ...
    <div className="app">
        ...
        <div className={`topbar-search${mobileSearchOpen ? '' : ' topbar-search--collapsed'}`}>
          <UnifiedSearchInput
            ...
            onFocusChange={focused => {
              if (!focused && !searchQuery) setMobileSearchOpen(false);
            }}
          />
        </div>
          ...
          <div className="topbar-chips">
            ...
            {hasActiveFilters && (
              ...
              >
                ...
                <span>
                  {contextLabels
                    .filter(label => label && !label.includes('km') && label !== cityName)
                    .join(' · ') || 'Filters'}
                </span>
              </button>
            )}
        ...
        {panel === 'library' && !editingPlace && (
          <div className="panel library-panel">
            ...
            <div className="library-switcher" role="tablist" aria-label="Library sections">
              <button
                type="button"
                className={libraryTab === 'places' ? 'active' : ''}
                onClick={() => setLibraryTab('places')}
                role="tab"
                aria-selected={libraryTab === 'places'}
              >
                Places <span>{places.length}</span>
              </button>
              <button
                type="button"
                className={libraryTab === 'trips' ? 'active' : ''}
                onClick={() => setLibraryTab('trips')}
                role="tab"
                aria-selected={libraryTab === 'trips'}
              >
              ...
              <>
                <div className="library-places-header">
                  <div>
                    <h3>Saved places</h3>
                    <span>
                      {libraryPlaces.length}
                      {libraryPlaces.length !== places.length ? ` of ${places.length}` : ''} places
                    </span>
                  </div>
                  <button
                    type="button"
      ...
      {isMobile && selectedPlace && (
        <BottomSheet
          ...
          onDelete={handleDeletePlace}
          onToggleVisited={(id, visited) => {
            updatePlace(id, { visited } as Partial<PlaceInput>);
            setSelectedPlace(prev => (prev?.id === id ? { ...prev, visited } : prev));
          }}
          confirmingId={confirmingId}
